...
/**
 * @title FrothComicTournamentV3
 * @dev Daily comic tournament with continuous voting and automatic finalization
 * @notice V3 UPDATE: Allows updating ComicNFT address if needed
 */
contract FrothComicTournamentV3 is Ownable {
    ...
    struct DayData {
        ...
        mapping(address => uint256) votingPowerOnWinner;
    ...
    event ComicNFTUpdated(address indexed oldAddress, address indexed newAddress);
    ...
    /**
     * @notice V3 UPDATE: Can now update ComicNFT address if needed
     * @dev Only owner can call, allows fixing misconfigurations
     */
    function setComicNFT(address _comicNFT) external onlyOwner {
        require(_comicNFT != address(0), "Invalid address");
        address oldAddress = comicNFT;
        comicNFT = _comicNFT;
        emit ComicNFTUpdated(oldAddress, _comicNFT);
    }
