...
/**
 * @title ProfileNFTFixed
 * @dev Complete tiered identity verification system with all original features + fixes
 */
contract ProfileNFTFixed is ERC721, AccessControl, ReentrancyGuard {
    ...
    /**
     * @dev FIXED: Create basic Tier 0 profile with proper fee handling
     */
    function createBasicProfile(
        ...
        // FIXED: Proper fee validation using isQualified function
        bool hasQualifyingTokens = false;
        
        try tokenQualifier.isQualified(msg.sender) returns (bool qualified) {
        ...
        if (hasQualifyingTokens) {
            ...
            uint256 requiredFee = tokenQualifier.getRequiredFee(TIER_BASIC);
    ...
    function getBasicProfileFee() external view returns (uint256) {
        return tokenQualifier.getRequiredFee(TIER_BASIC);
    }
